...
namespace CareerCloud.BusinessLogicLayer
{
    public class CompanyJobDescriptionLogic : BaseLogic<CompanyJobDescriptionPoco>
    {
        ...
        {
            CompanyJobDescriptionPoco poco = base.GetAll().Where(s => s.Login == userName).FirstOrDefault();
            if (null == poco)
            {
                return false;
            }
        ...
        protected override void Verify(CompanyJobDescriptionPoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(300, $"Job name {poco.Job_Name} is required"));
                }
                if (string.IsNullOrEmpty(poco.Job_Descriptions))
                {
                    exceptions.Add(new ValidationException(301, $"Job descriptions {poco.Job_Descriptions} is required"));
                }
            }

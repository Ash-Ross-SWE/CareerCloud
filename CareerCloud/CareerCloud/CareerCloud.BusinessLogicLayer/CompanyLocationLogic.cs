    ...
    public class CompanyLocationLogic : BaseLogic<CompanyLocationPoco>
    {
        ...
        protected override void Verify(CompanyLocationPoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(500, $"Country code {poco.CountryCode} cannot be empty"));
                }
                if (string.IsNullOrEmpty(poco.Province))
                {
                    exceptions.Add(new ValidationException(501, $"Province {poco.Province} cannot be empty"));
                }
                if (string.IsNullOrEmpty(poco.Street))
                {
                    exceptions.Add(new ValidationException(502, $"Street {poco.Street} cannot be empty"));
                }
                if (string.IsNullOrEmpty(poco.City))
                {
                    exceptions.Add(new ValidationException(503, $"City {poco.City} cannot be empty"));
                }
                if (string.IsNullOrEmpty(poco.PostalCode))
                {
                    exceptions.Add(new ValidationException(504, $"Postal Code {poco.PostalCode} cannot be empty"));
                }
            }
